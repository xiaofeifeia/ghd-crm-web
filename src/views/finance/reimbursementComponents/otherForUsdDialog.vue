<template>
  <div class="fixed_table">
    <!-- 评价帮费用 输入dialog-->
    <el-dialog title="pz费用" :visible.sync="inputDialogVisible" width="90%" :before-close="closeDialog" v-loading="fullScreenLoading" v-dialogDrag top="6vh">
      <el-form :model="otherForUsdForm" :rules="rules" ref="otherForUsdForm">
        <el-table class="inputMainClass" :data="otherForUsdForm.otherForUsdList" fit highlight-current-row
                  :header-cell-style="{ background: '#f2f2f2', color: '#333',fontSize:'14px'}" :height="tableHeight">
          <el-table-column label="#" type="index" align="center" width="65px" fixed="left"></el-table-column>
          <el-table-column label="日期" align="center" width="240px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.date'" :rules="rules.date">
                <el-date-picker v-model="scope.row.date" size="mini" type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" @input="changeList(scope.row, 1)"></el-date-picker>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="亚马逊订单号" align="center" width="240px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.orderId'" :rules="rules.orderId">
                <el-input v-model="scope.row.orderId" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)" />
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="ASIN" align="center" width="170px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.asin'" :rules="rules.asin">
          ...
          <el-table-column label="SKU" align="center" width="170px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.sku'" :rules="rules.sku">
                <el-input v-model="scope.row.sku" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="收款微信ID/支付宝" align="center" width="210px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.payeeName'" :rules="rules.payeeName">
                <el-input v-model="scope.row.payeeName" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="付款微信ID/支付宝" align="center" width="150px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.payerName'" :rules="rules.payerName">
                <el-input v-model="scope.row.payerName" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="品牌" align="center" width="150px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.brand'" :rules="rules.brand">
                <el-input v-model="scope.row.brand" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="主管" align="center" width="160px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.leader'" :rules="rules.leader">
                <el-input v-model="scope.row.leader" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="组别" align="center" width="170px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.team'" :rules="rules.team">
                <el-input v-model="scope.row.team" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="渠道" align="center" width="170px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.channel'" :rules="rules.channel">
          ...
          <el-table-column label="国家" align="center" width="100px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.country'" :rules="rules.country">
                <el-input v-model="scope.row.country" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="review链接" align="center" width="200px" show-overflow-tooltip>
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.link'">
          ...
          <el-table-column label="价格" align="center" width="150px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.price'" :rules="rules.price">
                <el-input v-model="scope.row.price" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="价格币种" align="center" width="100px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.priceCurrency'" :rules="rules.priceCurrency">
                <el-input v-model="scope.row.priceCurrency" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="佣金" align="center" width="150px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.commission'" :rules="rules.commission">
                <el-input v-model="scope.row.commission" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="佣金币种" align="center" width="130px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.commissionCurrency'" :rules="rules.commissionCurrency">
          ...
          <el-table-column label="手续费" align="center" width="150px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.serviceFee'" :rules="rules.serviceFee">
                <el-input v-model="scope.row.serviceFee" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="手续费币种" align="center" width="130px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.serviceFeeCurrency'" :rules="rules.serviceFeeCurrency">
                <el-input v-model="scope.row.serviceFeeCurrency" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="汇率" align="center" width="100px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.rate'" :rules="rules.rate">
                <el-input v-model="scope.row.rate" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="总金额(人民币)" align="center" width="160px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.total'">
                <el-input v-model="scope.row.total" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="总金额(美元)" align="center" width="160px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.totalUsd'">
                <el-input v-model="scope.row.totalUsd" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="备注" align="center" width="200px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.remark'" :rules="rules.remark">
                <el-input v-model="scope.row.remark" size="mini" placeholder="请输入" @input="changeList(scope.row, 1)"/>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="花费人" align="center" width="160px">
            <template slot-scope="scope">
              <el-form-item :prop="'otherForUsdList.' + scope.$index + '.applicant'" :rules="rules.applicant">
          ...
          <el-table-column label="操作" align="center" width="160px" fixed="right">
            <template slot-scope="scope">
              <el-button v-if="selectedRow.claimerId == userId" size="mini" type="danger" @click="deleteList(scope.row, 1)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-form>
      <div v-show="!fullScreenLoading" class="pagination-container">
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                       :current-page.sync="getOtherForUsdForm.page" :page-sizes="[10, 20, 30, 50, 100, 500]" :page-size="getOtherForUsdForm.limit"
                       layout="total, sizes, prev, pager, next, jumper" :total="otherForUsdForm.otherForUsdTotal"/>
      </div>
      <div style="margin-top: 15px;text-align: center">
        ...
        <el-button type="primary" v-waves @click="exportDetail">导出</el-button>
        ...
        <el-button v-if="selectedRow.claimerId == userId" v-waves @click="saveDetail">保存</el-button>
      </div>
    </el-dialog>

    <!-- 评价帮费用 查询dialog -->
    <el-dialog title="pz费用" :visible.sync="queryDialogVisible" width="90%" :before-close="closeDialog" v-loading="fullScreenLoading" v-dialogDrag top="3vh">
      <el-tabs v-model="activeTabName" type="card" @tab-click="handleTabClick">
        <el-tab-pane label="pz费用" name="allOtherForUsd">
          <el-table :data="otherForUsdForm.otherForUsdList" ref="otherForUsdQueryForm" fit highlight-current-row
                    :header-cell-style="{ background: '#f2f2f2', color: '#333',fontSize:'14px'}" :height="tableHeight">
            ...
            <el-table-column label="亚马逊订单号" align="center" width="210px">
              <template slot-scope="scope">
            ...
            <el-table-column label="ASIN" align="center" width="140px" prop="asin"/>
            <el-table-column label="SKU" align="center" width="140px">
              <template slot-scope="scope">
            ...
            <el-table-column label="收款微信ID/支付宝" align="center" width="210px" prop="payeeName" show-overflow-tooltip/>
            <el-table-column label="付款微信ID/支付宝" align="center" width="210px" prop="payerName" show-overflow-tooltip/>
            <el-table-column label="品牌" align="center" width="150px" prop="brand"/>
            ...
            <el-table-column label="国家" align="center" width="100px" prop="country"/>
            <el-table-column label="review链接" align="center" width="130px" show-overflow-tooltip prop="link"/>
            <el-table-column label="价格" align="center" width="100px" prop="price"/>
            <el-table-column label="价格币种" align="center" width="100px" prop="priceCurrency"/>
            <el-table-column label="佣金" align="center" width="100px" prop="commission"/>
            <el-table-column label="佣金币种" align="center" width="100px" prop="commissionCurrency"/>
            <el-table-column label="手续费" align="center" width="100px" prop="serviceFee"/>
            <el-table-column label="手续费币种" align="center" width="100px" prop="serviceFeeCurrency"/>
            <el-table-column label="汇率" align="center" width="100px" prop="rate"/>
            <el-table-column label="总金额(人民币)" align="center" width="100px" prop="total"/>
            <el-table-column label="总金额(美元)" align="center" width="100px" prop="totalUsd"/>
          ...
          <div v-show="!fullScreenLoading" class="pagination-container">
            <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                           :current-page.sync="getOtherForUsdForm.page" :page-sizes="[10, 20, 30, 50, 100, 500]"
                           :page-size="getOtherForUsdForm.limit" layout="total, sizes, prev, pager, next, jumper"
                           :total="otherForUsdForm.otherForUsdTotal"/>
          </div>
          <div align="center">
            <el-button type="primary" v-waves @click="exportDetail">导出</el-button>
        ...
        <el-tab-pane label="订单抽查" name="checkOrder">
          <el-form :model="orderCheckForm" :rules="orderCheckRules" ref="orderCheckForm">
            <el-table :data="orderCheckForm.orderCheckList" ref="orderCheckTable" fit highlight-current-row
                      :header-cell-style="{ background: '#f2f2f2', color: '#333',fontSize:'14px'}" :height="tableHeight">
              <el-table-column type="selection" align="center" width="65px" fixed="left"/>
              <el-table-column label="#" type="index" align="center" width="65px" fixed="left"/>
              <el-table-column label="审批备注" align="center" width="200px">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.subtractRemark" size="mini" placeholder="请输入" @input="changeList(scope.row, 2)"/>
                </template>
              </el-table-column>
              <el-table-column width="80px" align="center" label="订单截图">
                <template slot-scope="scope">
              ...
              <el-table-column label="订单截图命名" align="center" width="200px">
                <template slot-scope="scope">
                  <el-form-item :prop="'orderCheckList.' + scope.$index + '.orderScreenshot'" :rules="orderCheckRules.orderScreenshot">
                    <el-input v-model="scope.row.orderScreenshot" size="mini" placeholder="请输入" @input="changeList(scope.row, 2)"/>
                  </el-form-item>
                </template>
              </el-table-column>
              <el-table-column label="日期" align="center" width="130px">
              ...
              <el-table-column label="亚马逊订单号" align="center" width="210px" prop="orderId"/>
              <el-table-column label="ASIN" align="center" width="140px" prop="asin"/>
          ...
          <div align="center" style="margin-top: 10px">
            ...
            <el-button v-if="selectedRow.claimerId == userId && selectedRow.status != 5" type="primary" v-waves @click="openImageDialog(4)">上传订单截图</el-button>
            <el-button v-if="selectedRow.status != 5" type="primary" v-waves @click="addFeeSubtract(2)">加入扣减</el-button>
            <el-button type="primary" v-waves @click="closeDialog">取消</el-button>
            <el-button v-if="selectedRow.claimerId == userId && selectedRow.status != 5" type="primary" v-waves @click="saveCheckedOtherForUsdDetail">保存</el-button>
          </div>
        </el-tab-pane>
        <el-tab-pane label="费用扣减" name="feeSubtract">
          <el-form :model="feeSubtractForm">
            <el-table :data="feeSubtractForm.feeSubtractList" ref="feeSubtractForm" fit highlight-current-row
              ...
              <el-table-column label="亚马逊订单号" align="center" width="230px">
                <template slot-scope="scope">
                  <span style="color:red; cursor:pointer; background-color:papayawhip; padding:10px" @click="viewDuplicateDetail(scope.row)" v-if="scope.row.duplicateNum >= 1">{{scope.row.orderId}}</span>
                  <span v-else>{{scope.row.orderId}}</span>
                </template>
              ...
              <el-table-column label="ASIN" align="center" width="140px" prop="asin"/>
              <el-table-column label="SKU" align="center" width="160px">
                <template slot-scope="scope">
                  <span style="color:red; cursor:pointer; background-color:papayawhip; padding:10px" @click="viewDuplicateDetail(scope.row)" v-if="scope.row.duplicateNum >= 1">{{scope.row.sku}}</span>
                  <span v-else>{{scope.row.sku}}</span>
                </template>
              ...
              <el-table-column label="品牌" align="center" width="150px" prop="brand"/>
              <el-table-column label="主管" align="center" width="150px" prop="leader"/>
              <el-table-column label="主管" align="center" width="150px" prop="team"/>
              <el-table-column label="渠道" align="center" width="150px" prop="channel"/>
              <el-table-column label="国家" align="center" width="100px" prop="country"/>
              <el-table-column label="review链接" align="center" width="130px" show-overflow-tooltip prop="link"/>
              <el-table-column label="价格" align="center" width="100px" prop="price"/>
    ...
    <el-dialog title="导入" :visible.sync="importDialogVisible" :before-close="closeImportDialog" v-loading="fullScreenLoading" v-dialogDrag>
      ...
      <div align="center">
        <el-upload style="display: inline-block;margin-left: 10px;margin-right: 10px" ref="uploadFile"
                   action="/api/finance/financeLoanReimbursement/importOtherForUsdData" name="excel" :headers="headers" :auto-upload='true' :limit="1"
    ...
    <el-dialog title="导入" :visible.sync="importCheckedOrderDialogVisible" :before-close="closeImportCheckedOrderDialog" v-loading="fullScreenLoading" v-dialogDrag>
      ...
      <div align="center">
        <el-upload style="display: inline-block;margin-left: 10px;margin-right: 10px" ref="uploadFile"
                   action="/api/finance/financeLoanReimbursement/importOtherForUsdCheckedOrder" name="excel" :headers="headers" :auto-upload='true' :limit="1"
    ...
    <!-- 评价帮费用 查重dialog-->
    <el-dialog title="重复的pz费用" :visible.sync="duplicateOtherForUsdVisible" width="90%" v-loading="fullScreenLoading" v-dialogDrag>
      <el-table :data="duplicateOtherForUsdList" border highlight-current-row :height="tableHeight">
        <el-table-column label="#" type="index" align="center" width="65px" fixed="left"/>
        <el-table-column label="核销单号" align="center" width="160px" prop="reimbursementNo"/>
        <el-table-column label="日期" align="center" width="130px">
          <template slot-scope="scope"><span>{{$moment(scope.row.date).format("YYYY-MM-DD")}}</span></template>
        </el-table-column>
        <el-table-column label="亚马逊订单号" align="center" width="180px" prop="orderId"/>
        <el-table-column label="ASIN" align="center" width="130px" prop="asin"/>
        <el-table-column label="SKU" align="center" width="140px" prop="sku"/>
        <el-table-column label="状态" align="center" width="170px" prop="statusStr"/>
        <el-table-column label="收款微信ID/支付宝" align="center" width="150px" prop="payeeName"/>
        <el-table-column label="付款微信ID/支付宝" align="center" width="150px" prop="payerName"/>
        <el-table-column label="品牌" align="center" width="150px" prop="brand"/>
        <el-table-column label="主管" align="center" width="150px" prop="leader"/>
        <el-table-column label="组别" align="center" width="150px" prop="team"/>
        <el-table-column label="渠道" align="center" width="150px" prop="channel"/>
        ...
        <el-table-column label="备注" align="center" width="200px" prop="remark"/>
        <el-table-column label="花费人" align="center" width="150px" prop="applicant"/>
        <el-table-column label="报销人" align="center" width="200px" prop="claimerName"/>
      </el-table>
    </el-dialog>
    ...
    <el-dialog :title="importErrorTitle" :visible.sync="importErrorDialogVisible" width="90%" v-loading="fullScreenLoading" v-dialogDrag top="6vh">
      <el-table :data="importErrorList" border fit highlight-current-row style="width: 100%" :height="tableHeight">
        ...
        <el-table-column width="130px" align="center" label="日期">
        ...
        <el-table-column width="180px" align="center" label="亚马逊订单号" prop="orderId"/>
        <el-table-column width="130px" align="center" label="asin" prop="asin"/>
        <el-table-column width="140px" align="center" label="sku" prop="sku"/>
        <el-table-column width="150px" align="center" label="收款微信ID/支付宝" prop="payeeName"/>
        <el-table-column width="150px" align="center" label="付款微信ID/支付宝" prop="payerName"/>
        <el-table-column width="150px" align="center" label="品牌" prop="brand"/>
        <el-table-column width="150px" align="center" label="主管" prop="leader"/>
        <el-table-column width="150px" align="center" label="组别" prop="team"/>
        <el-table-column width="150px" align="center" label="渠道" prop="channel"/>
        <el-table-column width="100px" align="center" label="国家" prop="country"/>
        <el-table-column width="130px" align="center" label="review链接" show-overflow-tooltip prop="link"/>
        <el-table-column width="100px" align="center" label="价格" prop="price"/>
        <el-table-column width="100px" align="center" label="价格币种" prop="priceCurrency"/>
        <el-table-column width="100px" align="center" label="佣金" prop="commission"/>
        <el-table-column width="100px" align="center" label="佣金币种" prop="commissionCurrency"/>
        <el-table-column width="100px" align="center" label="手续费" prop="serviceFee"/>
        ...
        <el-table-column width="100px" align="center" label="总金额(人民币)" prop="total"/>
        <el-table-column width="100px" align="center" label="总金额(美元)" prop="totalUsd"/>
        <el-table-column width="200px" align="center" label="备注" prop="remark"/>
        <el-table-column width="150px" align="center" label="花费人" prop="applicant"/>
      </el-table>
    </el-dialog>

    <!-- 上传付款截图 -->
    <el-dialog :visible.sync="importImgdialogVisible" v-loading="fullScreenLoading" v-dialogDrag>
      <!-- uploadImageType 1:review付款截图,2:非review付款截图，3:review订单截图 ,4:评价帮订单截图-->
  ...
  export default {
    ...
    data() {
      return {
        ...
        duplicateOtherForUsdVisible: false,
        ...
        rules: {
          ...
          // total: [{required: true, message: '请输入总金额(人民币)', trigger: 'blur'}, {validator: this.sumTotal, trigger: 'blur'}],
          // totalUsd: [{required: true, message: '请输入总金额(美元)', trigger: 'blur'}, {validator: this.sumTotalUsd, trigger: 'blur'}],
          remark: [{max: 255, message: '最大长度为255个字符', trigger: 'blur'}],
          applicant: [{max: 128, message: '最大长度为128个字符', trigger: 'blur'}, {pattern: /^(.+\..+)?$/, message: '请输入带后缀的文件名'}]
        },
        orderCheckRules: {
        ...
        importErrorFrom: '',
        importErrorList: [],
        duplicateOtherForUsdList: [],
        tableHeight: undefined,
        activeTabName: 'allOtherForUsd',
        imgList: [],
    ...
    watch: {
      visible(val) {
        ...
        if (this.selectedRow.status == -1 || this.selectedRow.status == 0) {
          ...
          this.activeTabName = 'allOtherForUsd';
      ...
      dialogVisible(val) {
        if (val === false) {
          this.otherForUsdForm = {reimbursementId: this.selectedRow.id, otherForUsdList: [], changeList: [], deleteList: [], otherForUsdTotal: undefined};
        }
        this.$emit('change-otherForUsd-visible', val);
      },
    ...
    methods: {
      changeList(row, type) {
        ...
        let changeList;
        if (type === 1) {
          changeList = this.otherForUsdForm.changeList;
        } else if (type === 2) {
      ...
      deleteList(row) {
        let form = this.otherForUsdForm;
        let list = this.otherForUsdForm.otherForUsdList;
      ...
      getDetail() {
        this.getOtherForUsdForm.reimbursementId = this.selectedRow.id;
        // type 类型 1 review,2 非review，3报销单子项，4 评价帮
        this.getOtherForUsdForm.type = 4;
        this.fullScreenLoading = true;
        getReimbursementItemDetail(this.getOtherForUsdForm).then(response => {
          this.otherForUsdForm.otherForUsdList = response.data.rows;
          this.otherForUsdForm.otherForUsdTotal = response.data.total;
          this.sortIndex(1);
      ...
      clearDetail() {
        this.$confirm("是否清空费用明细?", "清空确认", {
          ...
          this.fullScreenLoading = true;
          // type 1:review明细，2:非review明细, 3:评价帮
          let form = {type: 3, reimbursementId: this.selectedRow.id};
      ...
      handleSizeChange(val) {
        this.getOtherForUsdForm.limit = val;
        this.getDetail();
      },
      handleCurrentChange(val) {
        this.getOtherForUsdForm.page = val;
        this.getDetail();
      },
      addFormColumn() {
        this.otherForUsdForm.otherForUsdList.push({});
        this.sortIndex(1);
      },
      sortIndex(type) {
        let list;
        if (type === 1) {
          list = this.otherForUsdForm.otherForUsdList;
        } else if (type === 2) {
      ...
      downloadTemplate() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['日期', '亚马逊订单号', 'ASIN', 'SKU', '收款微信ID/支付宝', '付款微信ID/支付宝', '品牌', '主管', '组别', '渠道', '国家', 'review链接', '价格', '价格币种', '佣金', '佣金币种', '手续费', '手续费币种', '汇率', '总金额(人民币)', '总金额(美元)', '备注', '花费人'];
          const data = [];
          excel.export_json_to_excel({
            header: tHeader,
            data,
            filename: 'pingjiabangTemplate',
            autoWidth: true,
            bookType: 'xlsx'
          });
          this.fullScreenLoading = false;
      ...
      saveDetail() {
        this.$refs['otherForUsdForm'].validate(valid => {
          if (valid) {
            this.fullScreenLoading = true;
            this.otherForUsdForm.reimbursementId = this.selectedRow.id;
            saveOtherForUsdDetail(this.otherForUsdForm).then(response => {
              if (response.resCode === 200) {
                this.$message.success("保存成功");
                this.otherForUsdForm.changeList = [];
                this.otherForUsdForm.deleteList = [];
              } else {
      ...
      getFeeSubtract() {
        // type 类型，1 review ，2 非review , 3 评价帮
        let form = {reimbursementId: this.selectedRow.id, type: 3};
        this.fullScreenLoading = true;
      ...
      viewDuplicateDetail(row) {
        ...
        findDuplicateDetail(query).then(response => {
          if (response.resCode === 200) {
            this.duplicateOtherForUsdList = response.data;
            this.duplicateOtherForUsdVisible = true;
          } else {
      ...
      addCheckedOrder() {
        let selection = this.$refs.otherForUsdQueryForm.selection;
      ...
      addFeeSubtract(type) {
        let selection;
        if(type === 1) {
          selection = this.$refs.otherForUsdQueryForm.selection;
        } else if (type === 2) {
        ...
        // type 类型 1.review 2.非review 3.评价帮
        let form = {type: 3, feeSubtractList: feeSubtractList};
      ...
      async exportDetail() {
        ...
        const res = await exportByReimbursementId({type: 4, reimbursementId: this.selectedRow.id}).catch(() => loading.close());
      ...
      async exportCheckedOrder() {
        ...
        // type 类型 1.review 2对公 3 gift card
        const res = await exportCheckedOrder({type: 2, reimbursementId: this.selectedRow.id}).catch(() => loading.close());
        if(res.data.type === 'application/json') {
          // 导出报错
          loading.close();
          const reader = new FileReader();
      ...
      async exportFeeSubtract() {
        ...
        const res = await exportFeeSubtract({type: 3, reimbursementId: this.selectedRow.id}).catch(() => loading.close());
        if(res.data.type === 'application/json') {
          // 导出报错
          loading.close();
          const reader = new FileReader();
      ...
      saveCheckedOtherForUsdDetail() {
        this.$refs['orderCheckForm'].validate(valid => {
          if (valid) {
            this.fullScreenLoading = true;
            saveOtherForUsdDetail(this.orderCheckForm).then(response => {
              if (response.resCode === 200) {
                this.$message.success("保存成功");
              } else {
                this.importErrorTitle = '保存失败';
      ...
      cancelFeeSubtract(row) {
        ...
        row.subtractRemark = '';
        // type 类型，1 review , 2 非review, 3 评价帮
        let form = {feeSubtractList: [row], type: 3};
      ...
      confirmFeeSubtract() {
        ...
        let form = {
          // type 扣减类型，1 review，2 非review,3 评价帮
          type: 3,
      ...
      saveFeeSubtract() {
        this.fullScreenLoading = true;
        // type 类型，1 review ，2 非review，3 评价帮
        let form = {feeSubtractList: this.feeSubtractForm.feeSubtractList, type: 3};
        updateFeeSubtract(form).then(response => {
          if (response.resCode === 200) {
            this.$message.success("保存成功");
          } else {
