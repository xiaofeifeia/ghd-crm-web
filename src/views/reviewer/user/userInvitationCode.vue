<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-button
        class="filter-item"
        v-if="userInvitationCodeManager_btn_add"
    ...
    >
      ...
      <el-table-column
        width="200px"
        align="center"
        label="邀请码状态#0：首次生成验证码；1：邮件推送验证码给客户；2：客户通过验证码已注册"
      >
        <template slot-scope="scope">
      ...
      <el-table-column fixed="right" align="center" label="操作" width="150">
        <template slot-scope="scope">
          <el-button
            v-if="userInvitationCodeManager_btn_edit"
            size="small"
            type="success"
            @click="handleUpdate(scope.row)"
          >编辑</el-button>
          <el-button
            v-if="userInvitationCodeManager_btn_del"
            size="small"
            type="danger"
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :model="form" :rules="rules" ref="form" label-width="100px">
        ...
        <el-form-item label="邀请码状态#0：首次生成验证码；1：邮件推送验证码给客户；2：客户通过验证码已注册" prop="status">
          <el-input
            v-model="form.status"
            placeholder="请输入邀请码状态#0：首次生成验证码；1：邮件推送验证码给客户；2：客户通过验证码已注册"
          ></el-input>
        </el-form-item>
...
import {
  ...
  delObj,
  putObj
} from "api/reviewer/user/userInvitationCode";
...
export default {
  ...
  data() {
    return {
        ...
        status: [
          {
            required: true,
            message:
              "请输入邀请码状态#0：首次生成验证码；1：邮件推送验证码给客户；2：客户通过验证码已注册",
            trigger: "blur"
          },
          {
            min: 3,
      ...
      dialogStatus: "",
      userInvitationCodeManager_btn_edit: false,
      userInvitationCodeManager_btn_del: false,
      userInvitationCodeManager_btn_add: false,
      textMap: {
        update: "编辑",
        create: "创建"
      },
  ...
  created() {
    this.getList();
    this.userInvitationCodeManager_btn_edit = this.elements[
      "userInvitationCodeManager:btn_edit"
    ];
    this.userInvitationCodeManager_btn_del = this.elements[
      "userInvitationCodeManager:btn_del"
    ];
    this.userInvitationCodeManager_btn_add = this.elements[
      "userInvitationCodeManager:btn_add"
    ];
  },
  computed: {
    ...mapGetters(["elements"])
